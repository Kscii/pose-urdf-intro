...
---
class: lecture-slide axis-color-slide
---

<div class="doc-section">05 · 坐标轴与 RPY</div>
...
<div class="rpy-card">
  <div class="rpy-card-heading"><strong>RPY = Roll–Pitch–Yaw</strong><span>一种欧拉角表示</span></div>
  <div class="rpy-rows">
    <div><code>R</code><strong>Roll · 横滚</strong><span>绕固定 X 轴</span></div>
    <div><code>P</code><strong>Pitch · 俯仰</strong><span>绕固定 Y 轴</span></div>
    <div><code>Y</code><strong>Yaw · 偏航</strong><span>绕固定 Z 轴</span></div>
  </div>
  <p>URDF 使用 <code>origin rpy="r p y"</code>，单位为 <strong>rad（弧度）</strong>。</p>
  <p class="rpy-example"><code>left_arm_joint6: rpy="-1.5708 0 3.1416"</code></p>
</div>
...
<!--
先用颜色识别 frame 的 XYZ 轴，再解释 URDF origin 中的 RPY。
-->
...
<div class="doc-section">06 · 静态 URDF 与动态 q</div>
...
<div class="doc-section">07 · 静态结构与动态数据会合</div>
...
<img class="inline-business-image h5-joint-image" src="./assets/source/h5关节值的图片.png" alt="H5 arm position 关节值矩阵">
...
<div class="doc-section">08 · URDF Tree 与 Chain</div>

# link 通过 joint 连接成树：以右手腕为例

<div class="urdf-tree-full-image">
  <img src="./assets/source/a2d-tf-tree-full.png" alt="A2D 完整 link tree">
</div>
<div class="image-note">从根节点 <code>base_link</code> 出发，身体、头部、左右机械臂和夹爪逐层分支；每个方框是一个 link，连接线对应 joint 建立的 parent → child 关系。</div>

<div class="doc-columns urdf-tree-detail">
<div>

```xml
<joint name="Joint_hand_r" type="fixed">
  <origin xyz="0 0 0" rpy="0 0 0"/>
  <parent link="Link7_r"/>
...
`right_base_link` 是右手腕的最后一个 link。虽然完整 URDF 会从身体继续分出头部、左臂和右臂等多条支路，但计算到某个目标 link 时，只取从根节点到它的唯一 parent → child 路径。

<div class="takeaway"><strong>关键点：</strong>link 是树的节点，joint 是连接节点的边；一条 End Chain 就是树中的一条有序路径。</div>

</div>
</div>

<!--
- 人形机器人的 link 不是一条直线，而是从 base_link 向头部、左右机械臂等方向分叉的树。
- 以右手腕为例，从根节点沿 parent → child 关系依次走到 right_base_link，就得到计算该 link 位姿所需的 chain。
-->
...
<!--
- 正运动学fk的作用就是使用关节空间求机器人末端在笛卡尔空间中的位姿。
也就是这里写的，获取机器人的一组关节角度值，然后基于urdf中的定义来算出最终机器人的夹抓或在什么位置，以及朝向什么方向
- 正运动学是可以求出唯一解的

- 逆运动学也就是ik， 就是使用末端位姿和urdf的定义求可以达到当前末端位姿需要的每个关节的关节角. 这里没有唯一解
- 我们这里主要使用的是正运动学
-->
...
<div class="doc-section">02 · world 与 base_link</div>
...
<div class="doc-section">03 · Raw End 校验</div>
...
<div class="doc-section">04 · End 类型</div>
...
<div class="doc-section">05 · TCP End</div>
